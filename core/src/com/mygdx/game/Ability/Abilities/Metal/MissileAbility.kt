...
class MissileAbility(override val attachedFightableObject: FightableObject): KeyAbility() {
    ...
    override fun frameAction() {
        if(currentFrame == 20){
            val soundGun = Missile(GameObjectData(x = pos.x.toInt(), y = pos.y.toInt()), size, attachedFightableObject.currentUnitVector, attachedFightableObject)
            soundGun.add()
        }
    }
}